...
function printUrl(id) {
  return `/message?message=${encodeURIComponent(id)}&print=true`;
}

function replyUrl(id, mode = "all") {
  return `/write?action=reply&message=${encodeURIComponent(id)}&mode=${mode}`;
}

function fwdUrl(id) {
  return `/write?action=forward&message=${encodeURIComponent(id)}`;
}

function secUrl(id) {
  return apiURL(`api/message_auth/?message=${encodeURIComponent(id)}`);
}

function calUrl(id, action, index) {
  return `/write?action=reply-cal-${action}&message=${encodeURIComponent(id)}&index=${index}`;
}
...
function handleAttachment(msg, attachment, index, summary) {
  if(attachment.content_type.includes("image")) {
    return (<a href={apiURL(`api/attachment/?message=${encodeURIComponent(msg.notmuch_id)}&num=${index}`)} target={getSetting("openInTab")} rel="noreferrer">
        <img src={apiURL(`api/attachment/?message=${encodeURIComponent(msg.notmuch_id)}&num=${index}&scale=1`)} alt={attachment.filename}/>
      </a>);
  } else if(attachment.content_type.includes("calendar") && attachment.preview !== null && summary === false) {
    return (<div><a href={apiURL(`api/attachment/?message=${encodeURIComponent(msg.notmuch_id)}&num=${index}`)} target={getSetting("openInTab")} rel="noreferrer">
    ...
    return (<a href={apiURL(`api/attachment/?message=${encodeURIComponent(msg.notmuch_id)}&num=${index}`)} target={getSetting("openInTab")} rel="noreferrer"><Icon icon={AttachFile}/>{attachment.filename}
      {summary ? "" : " (" + formatFSz(attachment.content_size) + ", " + attachment.content_type + ")"}
      </a>);
  }
}
...
export function Message(props) {
  ...
  async function changeTags(tagChanges) {
    props.sp?.(0);
    const response = await fetch(apiURL(`api/tag_batch/?type=message&ids=${encodeURIComponent(msg.notmuch_id)}&tags=${encodeURIComponent(tagChanges)}`));
    props.sp?.(1);
  ...
  mkShortcut([["w"]],
    // eslint-disable-next-line solid/reactivity
    () => { if(props.active) window.open(apiURL(`api/message_raw/?message=${encodeURIComponent(msg.notmuch_id)}`), getSetting("openInTab")); }
  );
          ...
          <Show when={msg.body["text/html"] === true}>
            <button class="toggle-content" data-testid={showHtml() ? "Text" : "HTML"} onClick={async (e) => {
                ...
                if(showHtml() && htmlContent() === undefined) {
                  ...
                  props.sp?.(0);
                  const response = await fetch(apiURL(`api/message_html/?message=${encodeURIComponent(msg.notmuch_id)}`));
                  props.sp?.(1);
                  if(!response.ok) {
